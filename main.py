...
user_info = db.users
event_info = db.events
attendance = db.attendance
friends_db = db.friends

user_info.ensure_index('user_id', cache_for=300, unique=True, drop_dups=True)
event_info.ensure_index('event_id', cache_for=300, unique=True, drop_dups=True)
attendance.ensure_index('uid')
attendance.ensure_index('eid')
attendance.ensure_index([('uid', ASCENDING), ('eid', ASCENDING)])
friends_db.ensure_index('user_id')
